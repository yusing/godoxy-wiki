...
`InboundMTLSProfile` references a named root-level inbound mTLS profile and enforces client-certificate validation for all HTTPS traffic on the entrypoint.

- When configured, route-level inbound mTLS overrides are not supported.
- Without a global profile, route-level inbound mTLS is selected by the TLS handshake SNI server name, using the same `FindRoute` lookup as HTTP routing.
