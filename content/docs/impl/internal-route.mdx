...
description: Provides HTTP routing, reverse proxy, file serving, and TCP/UDP stream proxying for GoDoxy
---
...
- Requests for secured routes must present matching SNI, and `Host` must resolve to the same route.
