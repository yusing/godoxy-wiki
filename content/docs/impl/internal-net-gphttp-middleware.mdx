...
For HTTP routes, a route-local middleware entry that contains only `bypass` and matches an existing entrypoint middleware name is treated as an overlay: its bypass rules are promoted into the effective entrypoint middleware for that route, and the route-local overlay is consumed so the same middleware is not evaluated twice.
