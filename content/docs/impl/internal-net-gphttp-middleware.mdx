...
For HTTP routes, any route-local middleware entry that sets `bypass` and matches an existing entrypoint middleware name contributes an overlay: its bypass rules are promoted into the effective entrypoint middleware for that route. When the route-local entry contains only `bypass`, it is consumed so the same middleware is not evaluated twice; entries with additional options still execute at route scope after promotion.
