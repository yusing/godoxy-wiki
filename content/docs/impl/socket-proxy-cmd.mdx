---
title: socket-proxy/cmd
description: The entry point for the Docker Socket Proxy sub-project, a secure proxy that exposes the Docker socket with fine-grained access control
---
...
Socket-proxy images build from the shared repository `Dockerfile` `socket-proxy` target:
