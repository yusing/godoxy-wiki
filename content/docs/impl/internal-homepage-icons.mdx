---
title: internal/homepage/icons
description: Icon URL parsing, fetching, and listing for the homepage dashboard
---
...
- Route scraping returns failures without background retries and caches results, including failures, for one minute. A later request after expiry can try again.
- Scraping uses a GoDoxy User-Agent and the route middleware pipeline, without a client IP. Client-address variables are empty; authentication-like middleware is skipped for these internal requests.
- HTML parsing fallback extracts `<link rel=icon>` from target pages
- HTML scrape runs only when the route health monitor reports healthy, so a dashboard favicon fetch cannot wake a napping or starting idle container
...
// ExpandRefs sanitizes refs and adds suffix-stripped candidates (immich-server -> immich)
func ExpandRefs(refs []string) []string

// Resolve finds catalog metadata and/or an icon URL from route references
func Resolve(refs []string) (u *URL, meta HomepageMeta, ok bool)
...
- Suffixed names (`immich-server`) match the base icon (`immich`)
...
func main() {
    ...
    fmt.Println(url.URL())
    // Output: https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/nginx.svg
    ...
    // Output: https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/nginx-dark.svg
}
```
...
- List tests mock HTTP with a test-local helper in `list/list_icons_test.go`
...
| WalkXCode     | `@walkxcode/nginx.svg`         | `https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/nginx.svg` |
