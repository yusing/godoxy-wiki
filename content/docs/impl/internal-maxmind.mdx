---
title: internal/maxmind
description: The maxmind package provides MaxMind GeoIP database integration for IP geolocation, including automatic database downloading and updates
---
...
instance, err := maxmind.New(parent, maxmindCfg)
if err != nil {
    log.Fatal(err)
}
...
silently inheriting a process-global reader from the previous runtime.
...
sequenceDiagram
    ...
    Config->>MaxMind: New(parent, config)
    ...
    participant Lookup
    Lookup->>MaxMind: LookupCity(runtime context, ip)
    MaxMind->>Cache: Check
    alt Cache Hit
