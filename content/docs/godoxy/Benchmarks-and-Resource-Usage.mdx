...
All rows below come from one full `shadowtree benchmark` run (four proxies) on **2026-05-03** (`Profile: smoke`). They are useful for relative comparison on this machine and profile, not for universal rankings.
...
```shell
# Profiles: smoke (default), stable, stress
shadowtree benchmark profile=stable
shadowtree benchmark profile=smoke
shadowtree benchmark profile=stress
...
# Combine typed args
shadowtree benchmark profile=stable h1=false h3=false
```

Selected typed arguments (defaults come from the active profile unless you override):

```shell
profile=smoke|stable|stress
...
h2load_duration=10                   # optional h2load duration seconds; derived from duration if empty
...
Built-in profile presets (from the Shadowtree recipe):
...
All three profiles use **4** `h2load` threads and set a non-zero warm-up before reused throughput (smoke **3s**, stable/stress **5s**). When **`runs` is greater than 1**, the recipe repeats each throughput benchmark and prints a median / coefficient-of-variation summary (`stable`, `stress`, or your own `runs`). Override profile defaults with the typed args above.
