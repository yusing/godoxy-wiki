...
description: Distribute traffic across multiple backend instances for high availability and scalability
---
...
<Callout type="info">
  Load balancing is currently supported for HTTP routes only. TCP/UDP load
  balancing is planned for future releases.
</Callout>
...
<Callout type="info">
  For IP hash mode, ensure the [RealIP
  middleware](middlewares/ip-resolution/Real-IP) is configured if GoDoxy
  operates behind a proxy.
</Callout>
