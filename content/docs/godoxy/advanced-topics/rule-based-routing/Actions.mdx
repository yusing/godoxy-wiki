...
| `middleware`               | `<name>` option block                   | conditional | Run the request phase of a built-in or composed middleware                                  | See [middleware](#middleware)                           |
...
A `middleware <name> { ... }` action runs the request phase of a built-in or
composed middleware at that point in rule execution. It continues to the next action
when the middleware permits the request. If the middleware handles the request
and stops the chain, later actions and the rule fallback do not run.

Only middleware with a request phase is supported. Response-only middleware,
such as `ModifyResponse`, is rejected when the rules are loaded. The name in the
block header selects the middleware. The block contains that middleware's
configuration properties and may include nested lists and mappings.


Use this action only in request-phase rules and action blocks. A `status` or
`resp_header` matcher is evaluated after the response is available. A nested
action block containing response-phase actions is also scheduled after the
response. Both cases are too late for request middleware and are rejected.
...
To configure middleware properties directly in a rule:
...
Every action can also be written as a command block. Its header contains the
command name followed by any header arguments owned by that command. The
YAML-like property body is also defined and validated by the command. For
scalar commands, the property names correspond to the arguments shown in the
table above.
...
- Each command owns its header arguments and accepted property shapes.
- Scalar command blocks do not take header arguments. All declared properties
  must be present.
- Unknown or duplicate property names are rejected.
- Scalar property values must be strings, numbers, booleans, or null unless the
  command accepts structured data. Middleware configuration, for example, may
  contain nested lists and mappings.
