...
Every action can also be written as a command block with named YAML options. The
option names are the same arguments shown in the table above, in positional
order. GoDoxy flattens the mapping into the normal positional argument list and
then runs the same action validator, so behavior is identical to the inline
form.
...
- Do not mix inline arguments with named options. Use either `notify info ntfy
  "title" "body"` or `notify { ... }`, not `notify info ntfy { ... }`.
