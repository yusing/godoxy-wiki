...
Glob patterns use [github.com/gobwas/glob](https://github.com/gobwas/glob).
GoDoxy compiles them **without separators**, so `*` and `?` match `/` the
same way they match any other character. `**` also matches any sequence;
with no separators it is equivalent to `*` for `/`.

| Pattern   | Description                                  | Example                       | Matches                                      | Non-matches                       |
| --------- | -------------------------------------------- | ----------------------------- | -------------------------------------------- | --------------------------------- |
| `*`       | Any sequence of characters, including `/`    | `/api/*`                      | `/api/users`, `/api/users/123`               | `/api`                            |
| `**`      | Any sequence of characters, including `/`    | `/static/**`                  | `/static/css/style.css`, `/static/js/app.js` | —                                 |
| `?`       | Any single character, including `/`          | `/file?.txt`                  | `/file1.txt`, `/fileA.txt`, `/file/.txt`     | `/file12.txt`, `/file/abc.txt`    |
| `[abc]`   | Set: match any listed character              | `/file[123].txt`              | `/file1.txt`, `/file2.txt`, `/file3.txt`     | `/file4.txt`, `/fileA.txt`        |
| `[!abc]`  | Negated set                                  | `/file[!123].txt`             | `/fileA.txt`, `/fileB.txt`                   | `/file1.txt`, `/file2.txt`        |
| `[a-z]`   | Single range                                 | `/file[a-z].txt`              | `/filea.txt`, `/filem.txt`, `/filez.txt`     | `/fileA.txt`, `/file1.txt`        |
| `[0-9]`   | Single numeric range                         | `/v[0-9]/api`                 | `/v1/api`, `/v5/api`, `/v9/api`              | `/v10/api`, `/va/api`             |
| `{a,b,c}` | Alternatives (comma-separated, no spaces)    | `/api/{users,posts,comments}` | `/api/users`, `/api/posts`, `/api/comments`  | `/api/products`, `/api/users/123` |
| `\*`      | Literal asterisk                             | `/file\*.txt`                 | `/file*.txt`                                 | `/fileabc.txt`                    |
| `\?`      | Literal question mark                        | `/file\?.txt`                 | `/file?.txt`                                 | `/filea.txt`                      |

A `[...]` class is **either** one range (`[a-z]`, `[0-9]`) **or** a set of
listed characters (`[abc]`, `[!_]`, `[-abcdef0123456789]`), not both. Mixed
forms such as `[a-f0-9]` or `[a-f0-9-]` do not compile. Put a literal `-` first
or last in a set (`[-abc]` or `[abc-]`). Unclosed `{` / `[` and a trailing `\`
are syntax errors.
...
# Match versioned API paths (one digit; * still matches nested segments)
path glob("/api/v[0-9]/*")

# Hex-and-hyphen name: list the set, do not mix ranges in one [...]
path glob("/users/[-abcdef0123456789]*/profile")

# Match date-based paths (each [0-9] is its own range)
...
# Exclude names whose first character is underscore
path glob("/images/[!_]*")
```
