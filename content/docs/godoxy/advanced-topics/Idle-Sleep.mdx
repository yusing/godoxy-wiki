---
title: Idle Sleep
description: Automatically sleep idle containers and wake them on traffic. Saves resources for infrequently used services
---
...
2. For an HTML navigation, GoDoxy returns the loading page immediately and starts one background wake operation
3. Dependencies start first (if configured)
4. Main container starts
5. The loading page receives startup progress over server-sent events
6. GoDoxy waits up to `wake_timeout` for readiness, then the loading page refreshes into the active service

Non-HTML requests, and all requests when `no_loading_page` is enabled, wait for wake-up so GoDoxy can proxy the original request after the service becomes ready. Canceling an HTML navigation after its loading page has been returned does not cancel the shared wake operation.

Idle start, ready, sleep, pause, and error transitions appear in the WebUI Live Activity feed.
...
See [Proxmox Integration](/docs/godoxy/getting-started/Proxmox) for detailed configuration options.
