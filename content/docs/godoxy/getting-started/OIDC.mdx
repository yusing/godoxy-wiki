...
description: Configure OIDC authentication for GoDoxy and per-application middleware
---
...
Protect individual routes with OIDC. Omitted options inherit from the global
`GODOXY_OIDC_*` variables above. You can also configure a standalone issuer
and credentials on the middleware; global OIDC does not need to be enabled.
...
services:
  app:
    labels:
      # Inherit issuer and credentials from GODOXY_OIDC_*
      proxy.#1.middlewares.oidc:
```

Standalone issuer (for example Authentik per-application issuer):
...
See [OIDC Middleware](../advanced-topics/middlewares/authentication-and-security/OIDC) for the full option list and more examples.
