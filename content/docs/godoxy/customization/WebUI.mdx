...
This configures how routes are displayed on the dashboard. You can configure these settings via Docker labels, route files, or directly in the WebUI.
...
Categories and icons are automatically detected from:

- Container image names and aliases
- Common service suffixes such as `-server`, `-web`, and `-app` (`immich-server` matches `immich`)
